...
const OrderHistory: React.FC = () => {
  const [, setData] = useState<orderHistory[]>([]);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_PUBLIC_API_URL}/order-history`,
        {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) throw new Error('Gagal mengambil data order history');
