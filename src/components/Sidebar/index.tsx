...
const Sidebar = ({ sidebarOpen, setSidebarOpen, role }: SidebarProps) => {
  ...
  const renderMenuByRole = (role: 'admin' | 'dealer') => {
    switch (role) {
      case 'admin':
        return (
          <>
  ...
  return (
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      <aside
        ref={sidebar}
        className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden ease-linear bg-white drop-shadow-1 dark:bg-boxdark lg:static lg:translate-x-0 ${
