...
const SignUp: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!password || password.trim() === '') {
      ...
      setPasswordError(
        'Kata sandi harus mengandung setidaknya 1 huruf kapital dan 1 simbol',
      );
      hasError = true;
    }

    // Validasi retype password
    if (!retypePassword || retypePassword.trim() === '') {
      setRetypePasswordError('Silakan ketik ulang kata sandi Anda');
      hasError = true;
    } else if (retypePassword !== password) {
      setRetypePasswordError('Kata sandi tidak cocok');
      hasError = true;
    }
    ...
    if (!hasError) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_APP_PUBLIC_API_URL}/auth/register`,
          {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
          ...
          // Navigasi ke halaman verifikasi kode OTP
          const userId = result.user_id; // Ambil user_id dari respons
          if (userId) {
            navigate(`/auth/verify-registration?user_id=${userId}`);
          } else {
            setPasswordError('Gagal mengambil ID pengguna. Silakan coba lagi');
            setLoading(false);
          }
        } else {
          setPasswordError(
            result.message || 'Akun sudah terdaftar. Silakan Masuk',
          );
          setLoading(false);
        }
      } catch (error) {
      ...
      <div className="flex min-h-screen flex-wrap items-center">
        <div className="hidden w-full xl:block xl:w-1/2">
          <div className="px-26 py-17.5 text-center">
            ...
            <p className="x-2xl:px-20">Belum punya akun? Silakan Daftar.</p>
